    ...
    if item != "":
        ...
        if item == "EUR/USD":
         ...
         readNews(item)
            ...
            messagebox.showinfo(title="Warning", message="Double click to choose currency")
...
#frame 3: text editor (fundamental analysis), scrool bar
...
#frame 2 tabview,  radiobuttn, button, result(labelframe), plot 
...
tabs.add(tab2, text="Scattered", compound=tk.LEFT)
...
buy_value = tk.Label(label_frame, text="0", bd=3)
...
def update():
    ...
    if method.get() == "m1":
        ...
        if item == "EUR/GBR":
            ...
            n=200
            long_rolling = moving_average(data_close_array, n)
            ax3.plot(range(n-1, len(data_close_array)), long_rolling, linestyle = "--", color="red")
            ax4.plot(range(n-1, len(data_close_array)), long_rolling, linestyle = "--", color="red")
